...
class BasePLModule(pl.LightningModule):
    ...
    def generate_triples(self,
        ...
        if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
            ...
            return [extract_triplets_typed(rel, self.entities) for rel in decoded_preds], [extract_triplets_typed(rel, self.entities) for rel in decoded_labels]
    ...
    def validation_epoch_end(self, output: dict) -> Any:
        if self.hparams.relations_file:
            ...
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                ...
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], list(self.relations.values()), "strict")            
            else:
